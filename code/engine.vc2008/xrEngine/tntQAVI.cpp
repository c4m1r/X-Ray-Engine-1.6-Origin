...
BOOL CAviPlayerCustom::DecompressFrame( DWORD dwFrameNum )
{
	...
	if( ICERR_OK != ICDecompressEx(m_aviIC, dwFlags, &m_biInFormat, (m_pMovieData + pCurrFrameIndex->dwChunkOffset + 8),
	0, 0, m_biInFormat.biWidth, m_biInFormat.biHeight,  &m_biOutFormat, m_pDecompressedBuf, 0, 0, m_biOutFormat.biWidth, m_biOutFormat.biHeight) ) { // range fix
...
VOID CAviPlayerCustom::PreRoll( DWORD dwFrameNum )
{
	...
	for( i=(int)dwFrameNum-1 ; i>0 ; i-- ) {
		...
		if( (int)m_dwFrameCurrent == i ) {
			...
			for( i++ ; i<(int)dwFrameNum ; i++ ) {
				...
				//res = ICDecompress(m_aviIC, dwFlags, &m_biInFormat, (m_pMovieData + pCurrFrameIndex->dwChunkOffset + 8) /*m_pCompressedBuf*/, &m_biOutFormat, m_pDecompressedBuf); //range fix
				res = ICDecompressEx(m_aviIC, dwFlags, &m_biInFormat, (m_pMovieData + pCurrFrameIndex->dwChunkOffset + 8),
	0, 0, m_biInFormat.biWidth, m_biInFormat.biHeight,  &m_biOutFormat, m_pDecompressedBuf, 0, 0, m_biOutFormat.biWidth, m_biOutFormat.biHeight); // range fix
	...
	res = ICDecompressEx(m_aviIC, ICDECOMPRESS_PREROLL | ICDECOMPRESS_HURRYUP, &m_biInFormat, (m_pMovieData + pCurrFrameIndex->dwChunkOffset + 8),
	0, 0, m_biInFormat.biWidth, m_biInFormat.biHeight,  &m_biOutFormat, m_pDecompressedBuf, 0, 0, m_biOutFormat.biWidth, m_biOutFormat.biHeight); // range fix
