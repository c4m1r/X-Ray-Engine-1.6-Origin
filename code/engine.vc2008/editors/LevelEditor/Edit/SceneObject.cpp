...
CEditableObject* CSceneObject::UpdateReference()
{
	...
	{
		 m_pReference = Lib.CreateEditObject(*m_ReferenceName);
	}
	else
		 m_pReference = 0;
...
bool CSceneObject::GetSummaryInfo(SSceneSummary* inf)
{
	...
	if (IsStatic()||IsMUStatic()){
        for(SurfaceIt 	s_it=E->m_Surfaces.begin(); s_it!=E->m_Surfaces.end(); s_it++){
			...
			inf->AppendTexture
			(
				AnsiString(ChangeFileExt(AnsiString(*(*s_it)->m_Texture),"").LowerCase()).c_str(),
				SSceneSummary::sttBase,area,pixel_area,
				E->m_LibName.c_str()
			);
        }
