 ...
 *
  * The affine transform is given as a \f$2\times 3\f$ matrix \( C \).
 * A pixel location \f$(x, y)\f$ in the source image is mapped to
 * \f$(x', y')\f$ in the destination image. The destination coordinates are:
 *
 * \f[
 * \begin{aligned}
 ...
 * \f]
 *
 * Affine transforms decompose into a linear part and a shift. The \f$2\times 2\f$ matrix
 * \f[
 * L = \begin{bmatrix}
 * c_{00} & c_{01} \\
 * c_{10} & c_{11}
 ...
 * \end{bmatrix}
 * \f]
 * is the post-transform translation; i.e., after applying \f$L\f$, translate by \f$v\f$.
 *  
 ...
 * The affine transform is given as a \f$2\times 3\f$ matrix \( C \).
 * A pixel location \f$(x, y)\f$ in the source image is mapped to
 * \f$(x', y')\f$ in the destination image. The destination coordinates satisfy:
 *
 * \f[
 * \begin{aligned}
 * x &= c_{00}\,x' + c_{01}\,y' + c_{02} \\
 ...
 * \f]
 *
 * In other words, given \f$C\f$, the source coordinates are transformed
 * to the destination using the inverse matrix \f$C^{-1}\f$:
 *
 * \f[
 * M = C^{-1} =
 * \begin{bmatrix}
 ...
 *
 * The perspective (projective) transform is given as a \f$3\times 3\f$ matrix \( C \).
 * A pixel location \f$(x, y)\f$ in the source image is mapped to \f$(x', y')\f$
 ...
 * \f]
 *
 * \note The denominator \f$c_{20}\,x + c_{21}\,y + c_{22}\neq 0\f$ for valid image points.
 **
 ...
 * Transforms (warps) an image based on a perspective transform. 
 *
  * The perspective (projective) transform is given by a \f$3\times 3\f$ matrix \( C \).
 * A pixel location \f$(x, y)\f$ in the source image maps to \f$(x', y')\f$ in the destination as:
 *
 * \f[
 * \begin{aligned}
 * x' &= \frac{c_{00}\,x + c_{01}\,y + c_{02}}{c_{20}\,x + c_{21}\,y + c_{22}}, \\
 ...
 * \f]
 *
 * Conversely, if \f$M = C^{-1}\f$, the destination coordinates \f$(x', y')\f$ map back to source:
 *
 * \f[
 * \begin{aligned}
 ...
 *       \f$c_{20}\,x + c_{21}\,y + c_{22} \neq 0\f$ and
 *       \f$m_{20}\,x' + m_{21}\,y' + m_{22} \neq 0\f$; also \f$\det(C)\neq 0\f$.
 *
