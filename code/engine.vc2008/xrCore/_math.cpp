...
//	DUMP_PHASE;
	...
	if (strstr(Core.Params,"-x86"))		{
		CPU::ID.features	&= ~(u32)(CpuFeature::Mmx);
		CPU::ID.features &= ~(u32)(CpuFeature::_3dNow);
		CPU::ID.features &= ~(u32)(CpuFeature::Sse);
		CPU::ID.features &= ~(u32)(CpuFeature::Sse2);
		CPU::ID.features &= ~(u32)(CpuFeature::Sse3);
		CPU::ID.features &= ~(u32)(CpuFeature::Ssse3);
		CPU::ID.features &= ~(u32)(CpuFeature::Sse41);
		CPU::ID.features &= ~(u32)(CpuFeature::Sse42);
	};

	string256	features;	xr_strcpy(features,sizeof(features),"RDTSC");
    if (CPU::ID.features & (u32)(CpuFeature::Mmx))	xr_strcat(features,", MMX");
    if (CPU::ID.features & (u32)(CpuFeature::_3dNow))	xr_strcat(features,", 3DNow!");
    if (CPU::ID.features & (u32)(CpuFeature::Sse))	xr_strcat(features,", SSE");
    if (CPU::ID.features & (u32)(CpuFeature::Sse2))	xr_strcat(features,", SSE2");
    if (CPU::ID.features & (u32)(CpuFeature::Sse3))	xr_strcat(features,", SSE3");
    if (CPU::ID.features & (u32)(CpuFeature::Ssse3))	xr_strcat(features,", SSSE3");
    if (CPU::ID.features & (u32)(CpuFeature::Sse41))xr_strcat(features,", SSE4.1");
    if (CPU::ID.features & (u32)(CpuFeature::Sse42))xr_strcat(features,", SSE4.2");
    if (CPU::ID.features & (u32)(CpuFeature::HT))	xr_strcat(features,", HTT");
...
void _initialize_cpu_thread	()
{
	...
	FPU::m24r	();
#endif  // XRCORE_STATIC
	if (CPU::ID.features& (u32)(CpuFeature::Sse))	{
