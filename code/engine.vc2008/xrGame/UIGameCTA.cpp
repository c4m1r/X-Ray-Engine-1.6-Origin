...
void CUIGameCTA::SetPlayerItemsToBuyMenu()
{
	...
	if (actor && !ps->testFlag(GAME_PLAYER_FLAG_VERY_VERY_DEAD))
	{
		...
		std::for_each(actor->inventory().m_belt.begin(), actor->inventory().m_belt.end(), std::function<void(PIItem const&)>(std::bind(
			static_cast<void(CUIGameCTA::*)(PIItem const&)>(&CUIGameCTA::BuyMenuItemInserter), this, std::placeholders::_1)));
		...
		std::for_each(actor->inventory().m_ruck.begin(), actor->inventory().m_ruck.end(), std::function<void(PIItem const&)>(std::bind(
			static_cast<void(CUIGameCTA::*)(PIItem const&)>(&CUIGameCTA::BuyMenuItemInserter), this, std::placeholders::_1)));
		...
		std::for_each(add_ammo.begin(), add_ammo.end(), std::function<void(aditional_ammo_t::value_type const&)>(std::bind(
			&CUIGameCTA::AdditionalAmmoInserter, this, std::placeholders::_1)));
