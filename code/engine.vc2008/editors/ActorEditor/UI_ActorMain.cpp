...
//---------------------------------------------------------------------------
CActorMain*&	AUI=(CActorMain*&)UI; // range fix
//---------------------------------------------------------------------------
...
CCommandVar CActorTools::CommandLoad(CCommandVar p1, CCommandVar p2)
{
    ...
    if(!p1.IsString()){
		temp_fn			= AnsiString(ChangeFileExt(m_LastFileName,"")).c_str(); // range fix
...
{
	return ATools->GetEditFileName().IsEmpty()?(char*)"noname":ATools->GetEditFileName().c_str(); // range fix
}

bool /*__fastcall*/ CActorMain::ApplyShortCut(WORD Key, TShiftState Shift) // range fix
{
    return inherited::ApplyShortCut(Key,Shift);
}
//---------------------------------------------------------------------------

bool /*__fastcall*/ CActorMain::ApplyGlobalShortCut(WORD Key, TShiftState Shift) // range fix
{
