...
//                UI->D3D_RenderNearer(0.0001);
                ...
                DU_impl.DrawPrimitiveL(D3DPT_LINESTRIP,m_RenderBuffer.size(),&*m_RenderBuffer.begin(),m_RenderBuffer.size(),dwColor,true,true); // range fixc
//                UI->D3D_ResetNearer();
            }break;
            case csPoint:{
            	FVF::TL pt[5];
...
void C3DCursor::GetRandomBrushPos(Fvector& pos, Fvector& norm){
	...
	float s_a = rand.randF(PI_MUL_2);
    float dist = sqrtf(rand.randF())*brush_radius;
