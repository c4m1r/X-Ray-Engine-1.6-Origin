...
public:
    ...
    IC shared_str&	_NameEditable		()      {return m_Name;}
...
typedef xr_vector< CSurface* > SurfaceVec; typedef SurfaceVec::iterator SurfaceIt;
typedef xr_vector< CEditableMesh* > EditMeshVec; typedef EditMeshVec::iterator EditMeshIt;
typedef xr_vector< COMotion* > OMotionVec; typedef OMotionVec::iterator OMotionIt;
typedef xr_vector< CSMotion* > SMotionVec; typedef SMotionVec::iterator SMotionIt;

struct ECORE_API SBonePart{
	shared_str 		alias;
    RStringVec 		bones;
};
typedef xr_vector< SBonePart > BPVec; typedef BPVec::iterator BPIt;
...
public:
    ...
    void			FillSurfaceProps		(CSurface* surf, LPCSTR pref, PropItemVec& items, ListItem* owner = NULL);
