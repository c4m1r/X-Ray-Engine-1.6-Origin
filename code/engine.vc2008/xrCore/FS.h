...
public:
	...
	IC void			r			(void *p,s32 cnt) {impl().r(p,cnt);}
...
public:
	...
	IC				IReader			(void *_data, s32 _size, s32 _iterpos=0)
	{
		data		= (char *)_data	;
		Size		= _size			;
		Pos			= 0				;
...
public:
	IC s32			elapsed		()	const		{	return Size-Pos;		};
	IC s32			tell		()	const		{	return Pos;				};
	IC void			seek		(s32 ptr)		{	Pos=ptr; VERIFY((Pos<=Size) && (Pos>=0));};
	IC s32			length		()	const		{	return Size;			};
	IC void*		pointer		()	const		{	return &(data[Pos]);	};
	IC void			advance		(s32 cnt)		{	Pos+=cnt;VERIFY((Pos<=Size) && (Pos>=0));};

public:
	void			r			(void *p,s32 cnt);
