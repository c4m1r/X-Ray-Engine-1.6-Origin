...
void game_cl_Deathmatch::SetBuyMenuItems		(PRESET_ITEMS* pItems, BOOL OnlyPreset)
{
	...
	if (pCurActor)
	{
		...
		std::for_each(add_ammo.begin(), add_ammo.end(), std::function<void(aditional_ammo_t::value_type const&)>(std::bind(&game_cl_Deathmatch::AdditionalAmmoInserter, this, std::placeholders::_1)));
			//std::bind(&game_cl_Deathmatch::AdditionalAmmoInserter, std::placeholders::_1));
	}
	else
	{
		//---------------------------------------------------------
