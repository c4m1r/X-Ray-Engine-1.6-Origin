...
typedef xr_vector< TElTreeItem* > ELVec; typedef ELVec::iterator ELVecIt;
...
{
    int cnt = 0;
	if (num_first) name.sprintf("%s_%02d",pref.c_str(),cnt++); else name = pref; // range fix
    while (FindItemInFolder(TYPE_FOLDER,tv,node,name))
		name.sprintf("%s_%02d",pref.c_str(),cnt++); // range fix
}
//---------------------------------------------------------------------------
