...
void TfrmDOShuffle::FillData()
{
	...
	OneColor->ShowIndex(this/*, DM, &color_indices*/);
...
{
    if (tvItems->Selected){
        AnsiString as = AnsiString(tvItems->Selected->Text);
...
void __fastcall TfrmDOShuffle::ebAppendIndexClick(TObject *Sender)
{
    ...
    color_indices.back()->ShowIndex(this/*, DM, &color_indices*/);
    //color_indices.back()->AddIndexColor();
}
//---------------------------------------------------------------------------
      ...
      TObject *Source, int X, int Y)
{
    ...
    if (OneColor&&OneColor->FDragItem){
        //OneColor->RemoveObjectToIndexColor(((AnsiString)OneColor->FDragItem->Text).c_str());
...
void __fastcall TfrmDOShuffle::createColorIndices()
{
   DM->RemoveColorIndices();
   for (u32 k=0; k<color_indices.size(); k++)
   {
