...
XRCORE_API void _dump_open_files(int mode)
{
	...
	if(bShow)
		Log("----total count=",(int)g_open_files.size());
}
	...
	for (++I; I != m_files.end(); ++I)
	{
		...
		if ((*end_symbol) != '\\')
		{
			...
			*/
			FS_File finalFile;
			if (flags & FS_ClampExt)
				finalFile.name = EFS.ChangeFileExt(entry_begin, "");
			else
				finalFile.name = entry_begin;
			u32 fl = entry.vfs != 0xffffffff ? FS_File::flVFS : 0;
			finalFile.size = entry.size_real;
			finalFile.time_write = entry.modif;
			finalFile.attrib = fl;
			dest.insert(finalFile);
		}
		else {
			// folder
			if ((flags & FS_ListFolders) == 0)	continue;
