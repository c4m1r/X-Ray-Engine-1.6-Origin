...
public:
    ...
    bool flush(LPCSTR fn)
    {
		...
		I->Create	(sx,sz,&*data.begin()); // range fix
        I->Vflip	();
...
BOOL SceneBuilder::BuildSun(u8 quality, Fvector2 dir)
{
	...
	{
		float _x = mn_x+x*da;  // range fix
		for (int y=0; y<samples; y++)
		{
			float _y = mn_y+y*da;  // range fix
            l_light_static.push_back(b_light_static());
            b_light_static& sl	= l_light_static.back();
			...
			sl.data.direction.setHP(_y,_x);  // range fix
        }
    }
    // dynamic
    {
...
void SceneBuilder::BuildPortal(b_portal* b, CPortal* e){
	...
	CopyMemory(&b->vertices[0],
		&e->m_SimplifyVertices[0],
		e->m_SimplifyVertices.size()*sizeof(Fvector)); // range fix
		/*e->m_SimplifyVertices.size()*sizeof(Fvector)*/
}
