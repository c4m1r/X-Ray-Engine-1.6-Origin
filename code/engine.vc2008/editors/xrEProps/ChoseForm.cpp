...
{
    currentCountSelectedItems = 0;
    tvItems->ShowCheckboxes 	= m_Flags.is(cfMultiSelect);
	int itm_cnt = _GetItemCount(m_LastSelection.c_str());
	if (m_Flags.is(cfMultiSelect)){  
                lblCurrentCountSelected->Caption = IntToStr(currentCountSelectedItems);
        lblMaxCountCheck->Caption = IntToStr(iMultiSelLimit);
        lblMaxCountTitle->Visible = true;
         lblCurrentCountTitle->Visible = true;
	    string256 T;
