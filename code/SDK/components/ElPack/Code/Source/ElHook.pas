...
type
  TCtlHook = class
  private
    FHooks : TElList; // TElList; --> range fix
    FControl : TControl;
{$ifndef CLX_USED}
{$IFDEF OLD_EL_HOOK}
...
type
  TElHookList = class
  private
    FCtlHooks : TElList; // TElList; --> range fix
{$ifndef CLX_USED}
    FHandle : THandle;
{$endif}    
...
begin
  inherited;
  FHooks := TElList.Create; // TElList.Create; --> range fix
{$ifndef CLX_USED}
{$IFDEF OLD_EL_HOOK}
...
procedure TElHookList.OnCtlHookDelete(Sender : TObject; Item : pointer); //--> range fix
begin
  TCtlHook(Item).Free;
end;
