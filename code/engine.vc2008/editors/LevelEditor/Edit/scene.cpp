...
//.				return false;
    ...
    if (bTestShaderCompatible){
    	bool res = true;
        ObjectList& lst = ListObj(OBJCLASS_SCENEOBJECT);
		typedef xr_set< CEditableObject* > EOSet; typedef EOSet::iterator EOSetIt;
        EOSet objects;
        int static_obj = 0; 
        for(ObjectIt it=lst.begin();it!=lst.end();it++){
...
{
        //Msg("EScene::SelectLightsForObject(CCustomObject* obj)");
...
void EScene::FillProp(LPCSTR pref, PropItemVec& items, ObjClassID cls_id)
{
	...
	// Camera
	PHelper().CreateFloat	(items,PrepareKey(pref,"Scene\\Viewport\\Near Plane"),				    &EPrefs->getViewNp(), 			0.01f,	10.f);
	PHelper().CreateFloat	(items,PrepareKey(pref,"Scene\\Viewport\\Far Plane"), 				    &EPrefs->getViewFp(),			10.f, 	10000.f);
	PHelper().CreateAngle	(items,PrepareKey(pref,"Scene\\Viewport\\FOV"),		  				    &EPrefs->getViewFov(),			deg2rad(0.1f), deg2rad(170.f));
...
void EScene::updateCamera()
{
	EDevice.m_Camera.SetViewport(EPrefs->getViewNp(), EPrefs->getViewFp(), rad2deg(EPrefs->getViewFov()));
}
