...
void CWeaponKnife::Load	(LPCSTR section)
{
	...
	m_Splash1HitsCount = 0; // pSettings->r_u32(section, "splash1_hits_count");
	m_Splash1PerVictimsHCount = 0; // pSettings->r_u32(section, "splash1_pervictim_hcount");
	m_Splash2HitsCount = 0; //	pSettings->r_u32(section, "splash2_hits_count");
/*#ifdef DEBUG
	m_dbg_data.m_pick_vectors.reserve(std::max(m_Splash1HitsCount, m_Splash2HitsCount));
#endif*/
	m_NextHitDivideFactor = 0;//	pSettings->r_float(section, "splash_hit_divide_factor");
